...
function limpiarInputs(){
    document.querySelector("#txtNomPelicula").value = "";
    document.querySelector("#txtComPelicula").value = "";
    document.querySelector("#txtRegistroPass").value = "";
    document.querySelector("#txtRegistroUser").value = "";
...
async function tokenValidoASYNC(){
...
async function getCategorias(){
    ...
    if(data.codigo == 200){
        ...
        Alertar("AVISO","","La sesión ha expirado.")
        Logout();
    }
}
...
function getCantUsuarios(p , listaUsuariosXPais){//recibe pais y una lista
    let objMarker = new Object();
    for(let lu of listaUsuariosXPais){ 
        if(lu.id === p.id){ //si el id del usuario y el id del pais son iguales?
            objMarker.id = p.id;
